import argparse
from lab_1 import convert_logs_to_dict, convert_logs_to_array
...
from lab_3 import process_logs_with_lvl, text_to_logging_lvl
...
def ipv4s():
    ...
    # python lab_5.py SSH_Test.txt -l "debug" ipv4s -l "Dec 10 06:55:48 LabSZ sshd[24200]: Failed password for invalid user webmaster from 173.234.31.186 port 38926 ssh2"
...
def message_type():
    converted_line = parse_ssh_log(args.log_line)
    print(get_message_type(converted_line))
...
def ssh_global_stats():
    converted_logs = convert_logs_to_dict(args.log_file)
    print(calculate_ssh_global_duration_statistics(converted_logs))
    # python lab_5.py SSH_Test.txt ssh-global-stats
...
def logging():
    logs = convert_logs_to_array(args.log_file)
    process_logs_with_lvl(text_to_logging_lvl(args.log_level), logs)
    # python lab_5.py SSH_Test.txt -l "error" logging
...
ssh_stats_parser = subparsers.add_parser("ssh-global-stats")
ssh_stats_parser.set_defaults(func=ssh_global_stats)
...
logging_parser.set_defaults(func=logging)
